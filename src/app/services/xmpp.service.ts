...
export class XmppService {
  ...
  private addMessageToHistory(jid: string, message: string, type: string) {
    ...
    if (!this.messagesHistory[jid]) {
      this.messagesHistory[jid] = [];
    }

    this.messagesHistory[jid].push({ message, type, timestamp });
  ...
  handleSubscriptionRequest(from: string) {
    if (!(from in this.roster)) {
      ...
      // Presentar la alerta para que el usuario decida aceptar o cancelar la solicitud
      this.alertService.presentAlertButtons(
        'Solicitud de suscripción',
        `¿Deseas aceptar la solicitud de suscripción de ${from}?`,
        () => this.acceptSubscription(from),  // Manejador para aceptar la suscripción
        () => this.rejectSubscription(from)   // Manejador para rechazar la suscripción
      );
